...
//if we remove arguments type annotations it shows any type
//if we remove just return type annotations works just fine

// This two functions just here to check the proper return type in the tests.
const courses = filterCourses(products)
const titles1dalja = getTitles(products)
...
//Looks for specific key values in the arguments we pass to the function.
...
const price1dalja = formatPrice(products[0])
console.log(price1dalja);
...
const anyProduct: Cat = {title: 'cat', name: 'Mr. Fluff', kind: 'British Shorthair', age: 4, price: 1}
...
const getTax = (course: Course) => {
    return (course.price as number) * 0.25;
}
...
console.log(removeCurrency(someAccount));
